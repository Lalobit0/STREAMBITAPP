...
function ModalServicio({ onGuardar, onCerrar, clienteNombre, clienteId, clientes }) {
  ...
  async function guardar() {
    ...
    const fechaISO = parseFechaInput(form.fecha)
    if (!fechaISO) return setError('Fecha inválida — usa DD/MM/AAAA')
    ...
    <Modal onClose={onCerrar}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:14}}>
        ...
        <div>
          ...
          <input value={form.fecha} onChange={e=>set('fecha',e.target.value)} placeholder="DD/MM/AAAA" maxLength={10} style={{fontFamily:'var(--mono)'}} />
        </div>
      </div>
...
function ModalRenovar({ servicio, cliente, onRenovar, onCerrar }) {
  const fechaBase = servicio.fecha_vencimiento || fechaHoy()
  const opciones = [
  ...
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const fechaFinal = manual ? parseFechaInput(fechaM) : sel

  async function renovar() {
    if (!fechaFinal) return setError('Fecha inválida')
    setSaving(true); setError('')
    try {
      const updates = { fecha_vencimiento: fechaFinal, estado: 'PENDIENTE', cobrado: null }
  ...
  return (
    <Modal onClose={onCerrar}>
      ...
      <div style={{fontSize:11,color:'var(--text3)',marginBottom:10,fontFamily:'var(--mono)'}}>FECHA ACTUAL: {formatFecha(servicio.fecha_vencimiento)}</div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:12}}>
        {opciones.map(o => (
          <button key={o.label} onClick={()=>{setSel(o.fecha);setManual(false)}}
            style={{background:!manual&&sel===o.fecha?'var(--bg3)':'var(--bg2)',border:`1px solid ${!manual&&sel===o.fecha?'var(--cyan)':'var(--border)'}`,borderRadius:10,padding:'10px 12px',cursor:'pointer',textAlign:'left',color:!manual&&sel===o.fecha?'var(--cyan)':'var(--text2)',transition:'all .15s'}}>
            <div style={{fontWeight:700,fontSize:13}}>{o.label}</div>
            <div style={{fontSize:11,fontFamily:'var(--mono)',marginTop:2,color:'var(--text3)'}}>{formatFecha(o.fecha)}</div>
          </button>
        ))}
      </div>
      <button onClick={()=>setManual(!manual)} className="btn btn-ghost" style={{width:'100%',justifyContent:'flex-start',marginBottom:manual?8:12}}>
        ✏️ Fecha manual {manual?'▲':'▼'}
      </button>
      {manual && <input value={fechaM} onChange={e=>setFechaM(e.target.value)} placeholder="DD/MM/AAAA" maxLength={10} style={{marginBottom:12,fontFamily:'var(--mono)'}} />}
      <div style={{marginBottom:14}}>
      ...
      <button className="btn btn-primary" style={{width:'100%',justifyContent:'center',padding:14,fontSize:15}} onClick={renovar} disabled={saving}>
        {saving ? 'Guardando...' : `Renovar → ${formatFecha(fechaFinal)}`}
      </button>
    </Modal>
  )
}
